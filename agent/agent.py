...
# Full communicaiton among all agents
class FC_Agents:
    def __init__(self, args):
        self.n_actions = args.n_actions
        self.n_agents = args.n_agents
    ...
    def get_action_weights(self, obs, last_action):
        ...
        weights, self.policy.eval_hidden, eval_me = self.policy.eval_rnn(inputs, self.policy.eval_hidden)

        eval_hidden = self.policy.eval_hidden.view(1, self.n_agents, -1)
        # two step processing
        # first get the corresponding \hat_{message}
        #eval_hidden_after = self.policy.eval_coord(eval_hidden)
        eval_hidden_after_0 = self.policy.eval_coord(eval_hidden[:, :2, :])
        eval_hidden_after_1 = self.policy.eval_coord(eval_hidden[:, 2:, :])
        eval_hidden_after = torch.cat([eval_hidden_after_0, eval_hidden_after_1], dim=1)

        # second process the hidden_before and after
        weights = self.policy.eval_Q_net(torch.cat((eval_hidden, eval_hidden_after), dim=-1))
        weights = weights.view(self.n_agents, -1)
        return weights.cpu()


    # 根据weights得到概率，然后再根据epsilon选动作
    def choose_action(self, weights, avail_actions, epsilon, maven_z=None, evaluate=False):
        avail_actions_ind = np.nonzero(avail_actions)[0]  # index of actions which can be choose
...
# Full communicaiton among all agents
class FC_ACML_Agents:
    def __init__(self, args):
        self.n_actions = args.n_actions
        self.n_agents = args.n_agents
        self.state_shape = args.state_shape
        self.obs_shape = args.obs_shape
        from policy.FC_ACML import FC
        self.policy = FC(args)
        self.args = args
